-- Fix Contact Form Trigger: Convert JSONB body to TEXT
-- The net.http_post might need body as TEXT, not JSONB directly
...
BEGIN
  ...
  -- Convert JSONB to TEXT for the HTTP request body
  ...
  RAISE NOTICE '[TRIGGER] Payload: %', payload_text;
  
  -- Make the HTTP request (body as TEXT converted from JSONB)
  SELECT net.http_post(
    ...
    body := payload_text::jsonb  -- Try both: convert TEXT back to JSONB
  ) INTO request_id;
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    RAISE WARNING '[TRIGGER ERROR] Payload was: %', payload_text;
    RETURN NEW;
END;
$$;
