...
async def create_booking_request_vapi(
    ...
    try:
        ...
        # If only start time is provided, set end time to same date with time 00:00:00 (end time unknown)
        # This allows VAPI to submit bookings when user only provides start time
        end_time_unknown = False
        if not requested_end_at or not requested_end_at.strip() or requested_end_at == "00" or requested_end_at == "00:00" or requested_end_at == "00:00:00":
            # Parse start time first to get the date
            try:
                temp_start_dt = _parse_datetime_robust(requested_start_at, "requested_start_at")
                # Set end time to start time + 30 minutes (default slot length)
                # This is a reasonable default for tour bookings
                temp_end_dt = temp_start_dt + timedelta(minutes=30)
                requested_end_at = temp_end_dt.isoformat()
                end_time_unknown = True
                print(f"⚠️  End time not provided or set to 00. Using default 30-minute slot: {requested_end_at}")
            except Exception as e:
                print(f"⚠️  Could not parse start time to set default end time: {e}")
                raise HTTPException(
                    status_code=400,
        ...
        # Robust datetime parsing
        print(f"🔍 Parsing dates: requested_start_at='{requested_start_at}', requested_end_at='{requested_end_at}', timezone='{tz_str}', end_time_unknown={end_time_unknown}")
        try:
        ...
        with Session(engine) as session:
            ...
            # If end time was auto-generated, add a note
            booking_notes = notes or ""
            if end_time_unknown:
                end_time_note = "[End time not provided by customer - using default 30-minute slot]"
                booking_notes = f"{end_time_note}\n{booking_notes}".strip() if booking_notes else end_time_note
            ...
            booking = PropertyTourBooking(
                ...
                customer_sent_end_at=requested_end_at if not end_time_unknown else None,  # Don't store auto-generated end time as "customer sent"
                status="pending",
                ...
                audit_log=[{
                    ...
                    "endTimeAutoGenerated": end_time_unknown
                }]
            )
